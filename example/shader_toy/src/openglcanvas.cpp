...
void OpenGLCanvas::CompileCustomFragmentShader(const std::string& customFragmentShaderSource) {
...
void OpenGLCanvas::OnPaint(wxPaintEvent& WXUNUSED(event)) {
    ...
    if (shaderProgram.shaderProgram.has_value()) {
        ...
        glUniform2f(glGetUniformLocation(shaderProgram.shaderProgram.value(), "iResolution"),
                    static_cast<float>(viewPortSize.x), static_cast<float>(viewPortSize.y));

        glUniform1f(glGetUniformLocation(shaderProgram.shaderProgram.value(), "iTime"), elapsedSeconds);
