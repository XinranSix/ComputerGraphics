...
const float CLICK_TOLRTANCE = 5.0f;
...
private:
    ...
    static Constraint* find_nearest_constraint(float mouse_x, float mouse_y,
        ...
        Constraint* nearest_constraint {};
        float min_distance = CLICK_TOLRTANCE;

        for (const auto& constraint : constraints) {
            float distance =
                point_to_segment_distance(mouse_x, mouse_y, constraint.p1->position.x, constraint.p1->position.y,
