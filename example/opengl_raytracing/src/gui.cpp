...
// The excessive use of the extern keyword here is probably not ideal. These functions should be declared in a header
// file but trying that resulted in linking errors whereas this works fine.
...
namespace GUI {
    ...
    void shaderVecParameter(const char* name, const char* displayName, float* floatPtr) {
        ...
        if (ImGui::InputFloat3(std::string("##").append(name).c_str(), floatPtr)) {
            if (Scene::boundShader)
                glUniform3f(glGetUniformLocation(Scene::boundShader, name), *(floatPtr + 0), *(floatPtr + 1),
                            *(floatPtr + 2));
            refreshRequired = true;
        }
    }
        ...
        if (Scene::selectedObjectIndex != -1) {
            ...
            if (ImGui::Checkbox(std::string("##").append(typeVariableName).c_str(), &isBox)) {
                ...
                if (isBox) {
                    ...
                    float minDimension = std::min(
                        { Scene::objects[i].scale[0], Scene::objects[i].scale[1], Scene::objects[i].scale[2] });
            ...
            if (ImGui::Checkbox("##plane_visible", &Scene::planeVisible)) {
                if (Scene::boundShader)
                    glUniform1ui(glGetUniformLocation(Scene::boundShader, "u_planeVisible"), Scene::planeVisible);
                refreshRequired = true;
            }
            ...
            shaderFloatParameter("u_planeMaterial.emissionStrength", "Emission Strength",
                                 &Scene::planeMaterial.emissionStrength);
            ...
            shaderSliderParameter("u_planeMaterial.specularHighlight", "Highlight",
                                  &Scene::planeMaterial.specularHighlight);
            shaderSliderParameter("u_planeMaterial.specularExponent", "Exponent",
                                  &Scene::planeMaterial.specularExponent);
        }
        ...
        for (int i = 0; i < Scene::lights.size(); i++) {
            ...
            if (ImGui::InputFloat3(std::string("##light_pos_").append(indexStr).c_str(), Scene::lights[i].position)) {
                if (Scene::boundShader)
                    glUniform3f(glGetUniformLocation(
                                ...
                                Scene::lights[i].position[0], Scene::lights[i].position[1],
            ...
            if (ImGui::InputFloat(std::string("##light_radius_").append(indexStr).c_str(), &Scene::lights[i].radius)) {
                if (Scene::boundShader)
                    glUniform1f(glGetUniformLocation(
                                    Scene::boundShader,
                                    std::string("u_lights[").append(std::to_string(i)).append("].radius").c_str()),
            ...
            if (ImGui::InputFloat(std::string("##light_power_").append(indexStr).c_str(), &Scene::lights[i].power)) {
                if (Scene::boundShader)
                    glUniform1f(glGetUniformLocation(
                                    Scene::boundShader,
                                    std::string("u_lights[").append(std::to_string(i)).append("].power").c_str()),
                                Scene::lights[i].power);
                refreshRequired = true;
            }
            ...
            if (ImGui::InputFloat(std::string("##light_reach_").append(indexStr).c_str(), &Scene::lights[i].reach)) {
                if (Scene::boundShader)
                    glUniform1f(glGetUniformLocation(
                                    Scene::boundShader,
                                    std::string("u_lights[").append(std::to_string(i)).append("].reach").c_str()),
                                Scene::lights[i].reach);
                refreshRequired = true;
            }
    ...
    void appSettingsUI() {
        ...
        if (ImGui::InputInt("##shadowResolution", &Scene::shadowResolution)) {
            if (Scene::boundShader)
                glUniform1i(glGetUniformLocation(Scene::boundShader, "u_shadowResolution"), Scene::shadowResolution);
            refreshRequired = true;
        }
        ...
        if (ImGui::InputInt("##lightBounces", &Scene::lightBounces)) {
            if (Scene::boundShader)
                glUniform1i(glGetUniformLocation(Scene::boundShader, "u_lightBounces"), Scene::lightBounces);
            refreshRequired = true;
        }
        ...
        if (ImGui::InputInt("##framePasses", &Scene::framePasses)) {
            if (Scene::boundShader)
                glUniform1i(glGetUniformLocation(Scene::boundShader, "u_framePasses"), Scene::framePasses);
            refreshRequired = true;
        }
        ...
        if (ImGui::InputFloat("##bloomRadius", &Scene::bloomRadius)) {
            if (Scene::boundShader)
                glUniform1f(glGetUniformLocation(Scene::boundShader, "u_bloomRadius"), Scene::bloomRadius);
            refreshRequired = true;
        }
        ...
        if (ImGui::InputFloat("##bloomIntensity", &Scene::bloomIntensity)) {
            if (Scene::boundShader)
                glUniform1f(glGetUniformLocation(Scene::boundShader, "u_bloomIntensity"), Scene::bloomIntensity);
            refreshRequired = true;
        }
    ...
    void skyboxSettingsUI() {
        ...
        if (ImGui::InputFloat("##skyboxStrength", &Scene::skyboxStrength)) {
            if (Scene::boundShader)
                glUniform1f(glGetUniformLocation(Scene::boundShader, "u_skyboxStrength"), Scene::skyboxStrength);
            refreshRequired = true;
        }
        ...
        if (ImGui::InputFloat("##skyboxGamma", &Scene::skyboxGamma)) {
            if (Scene::boundShader)
                glUniform1f(glGetUniformLocation(Scene::boundShader, "u_skyboxGamma"), Scene::skyboxGamma);
            refreshRequired = true;
        }
        ...
        if (ImGui::InputFloat("##skyboxCeiling", &Scene::skyboxCeiling)) {
            if (Scene::boundShader)
                glUniform1f(glGetUniformLocation(Scene::boundShader, "u_skyboxCeiling"), Scene::skyboxCeiling);
            refreshRequired = true;
        }
        ...
        if (ImGui::Button("Load")) {
            ...
            float* skyboxData = load_image_data(std::string("skyboxes\\").append(skyboxFilename).c_str(), &sbWidth,
    ...
    void animationRenderingUI() {
        ...
        if (ImGui::Button("Render")) {
            Animation::currentFrame = -1; // Set to -1 so the main loop can finish its current iteration before the
                                          // animation rendering process starts
    ...
    void render() {
        ...
        skyboxSettingsUI();
        cameraSettingsUI();
        if (animationRenderWindowVisible)
            animationRenderingUI();
