...
const GLfloat vertices[] = { -1.0F, -1.0F, 0.0F, 1.0F,  -1.0F, 0.0F, 1.0F,  1.0F,  0.0F,
...
GLuint directOutPassUniformLocation, accumulatedPassesUniformLocation, timeUniformLocation, camPosUniformLocation,
...
void mousebuttonCallback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_1 && action == GLFW_PRESS && !mouseAbsorbed &&
...
// Uses createShaderProgram to create a program with the correct constants depending on the Scene and reassigns
// everything that needs to be. If a program already exists, it is deleted.
...
void free_image_data(void* imageData) { stbi_image_free(imageData); }
...
bool handleMovementInput(GLFWwindow* window, double deltaTime, glm::vec3& cameraPosition, float& cameraYaw,
    ...
    *rotationMatrix =
        glm::rotate(glm::rotate(glm::mat4(1), cameraPitch, glm::vec3(1, 0, 0)), cameraYaw, glm::vec3(0, 1, 0));
...
void saveImage(GLFWwindow* w, int accumulatedPasses, const char* filepath) {
    ...
    for (int i = 0; i < bufferSize; i++)
        byteBuffer[i] = (unsigned char)(std::min(floatBuffer[i] / accumulatedPasses, 1.0f) * 255);
...
void renderAnimation(GLFWwindow* window, glm::vec3 posA, float yawA, float pitchA, glm::vec3 posB, float yawB,
                     float pitchB, int frames, int framePasses, int* renderedFrames) {
    ...
    for (int frame = 0; frame < frames; frame++) {
        ...
        glm::mat4 rotMatrix =
            glm::rotate(glm::rotate(glm::mat4(1), pitch, glm::vec3(1, 0, 0)), yaw, glm::vec3(0, 1, 0));
    ...
    while (!glfwWindowShouldClose(programWindow) && !GUI::shouldQuit) {
        ...
        if (Animation::currentlyRenderingAnimation) {
            if (Animation::currentFrame == -1)
                Animation::currentFrame = 0; // Setting currentFrame to -1 ensures we don't start writing frames before
                                             // this code has been called.
            ...
            if (glfwGetKey(programWindow, GLFW_KEY_ESCAPE) && glfwGetKey(programWindow, GLFW_KEY_LEFT_SHIFT))
                break;
        ...
        if (refreshRequired) {
            ...
            glUniform1i(accumulatedPassesUniformLocation,
                        accumulatedPasses); // If the shader receives a value of 0 for accumulatedPasses, it will
                                            // discard the buffer and just output what it rendered on that frame.
        }
        ...
        if (!mouseAbsorbed && !Animation::currentlyRenderingAnimation)
            GUI::render();
        ...
        // Because Animation::currentlyRenderingAnimation is set by the GUI, it will be true down here before it is
        // caught above. This is why GUI will initially set the currentFrame to -1 so this code knows it must not do
        // anything.
        if (Animation::currentlyRenderingAnimation && Animation::currentFrame >= 0) {
            if (Animation::currentPass >= Animation::framePasses - 1) {
                saveImage(programWindow, 1,
                          std::string("render_output\\")
                              .append(std::to_string(Animation::currentFrame))
                              .append(".png")
                              .c_str());
            ...
            if (glfwGetKey(programWindow, GLFW_KEY_ESCAPE))
                Animation::currentlyRenderingAnimation = false;
        }
    }
