...
void SceneBVH::build(std::vector<ShapeInstance>&& instances) {
    ...
    std::cout << "Mean Leaf Node ShapeInstance Count: "
              << static_cast<float>(instance_count) / static_cast<float>(state.leaf_node_count) << std::endl;
    ...
    for (size_t axis = 0; axis < 3; axis++) {
        ...
        for (const auto& instance : node->instances) {
            size_t bucket_idx = glm::clamp<size_t>(
                glm::floor((instance.center[axis] - node->bounds.b_min[axis]) * bucket_count / diag[axis]), 0,
                bucket_count - 1);
...
std::optional<HitInfo> SceneBVH::intersect(const Ray& ray, float t_min, float t_max) const {
    ...
    if (closest_instance) {
        ...
        closest_hit_info->normal = glm::normalize(
            glm::vec3(glm::transpose(closest_instance->object_from_world) * glm::vec4(closest_hit_info->normal, 0.f)));
