...
void BVH::build(std::vector<Triangle>&& triangles) {
    ...
    std::cout << "Mean Leaf Node Triangle Count: "
              << static_cast<float>(triangle_count) / static_cast<float>(state.leaf_node_count) << std::endl;
...
void BVH::recursiveSplit(BVHTreeNode* node, BVHState& state) {
    ...
    for (size_t axis = 0; axis < 3; axis++) {
        ...
        for (const auto& triangle : node->triangles) {
            ...
            size_t bucket_idx =
                glm::clamp<size_t>(glm::floor((triangle_center - node->bounds.b_min[axis]) * bucket_count / diag[axis]),
                                   0, bucket_count - 1);
...
std::optional<HitInfo> BVH::intersect(const Ray& ray, float t_min, float t_max) const {
    ...
    DEBUG_LINE(size_t bound_test_count {}, triangle_test_count {})
    ...
    while (true) {
        ...
        if (node.triangle_count == 0) {
            ...
            for (size_t i {}; i < node.triangle_count; ++i) {
    ...
    DEBUG_LINE(ray.bounds_test_count += bound_test_count)
