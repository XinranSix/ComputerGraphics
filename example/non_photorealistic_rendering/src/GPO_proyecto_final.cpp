...
void init_scene()
{
	...
	models[0] = cargar_modelo("./assets/spider.bix");  // Preparar datos de objeto, mandar a GPU
	textures[0] = cargar_textura("./assets/spider.jpg", GL_TEXTURE1); //GL_TEXTURE0 for postprocessing

	models[1] = cargar_modelo("./assets/halo.bix");
	textures[1] = cargar_textura("./assets/halo.jpg", GL_TEXTURE2);

	load_obj("./assets/helmet.obj", models[2]);
	textures[2] = cargar_textura("./assets/helmet.jpg", GL_TEXTURE3);

	load_obj("./assets/cat.obj", models[3]);
	textures[3] = cargar_textura("./assets/cat.jpg", GL_TEXTURE4);

	load_obj("./assets/jeep.obj", models[4]);
	load_obj("./assets/ball_fountain.obj", models[5]);
	load_obj("./assets/suzanne.obj", models[6]);
	...
	vertex_prog = leer_codigo_de_fichero("./shaders/base.vs");
	fragment_prog = leer_codigo_de_fichero("./shaders/base.fs");
	prog[0] = Compile_Link_Shaders(vertex_prog, fragment_prog);

	// PIXEL 2
	fragment_prog = leer_codigo_de_fichero("./shaders/pixel.fs");
	prog[1] = Compile_Link_Shaders(vertex_prog, fragment_prog);

	// TOON SHADING
	vertex_prog = leer_codigo_de_fichero("./shaders/phong.vs"); // same v.s. as phong
	fragment_prog = leer_codigo_de_fichero("./shaders/toon.fs");
	prog[2] = Compile_Link_Shaders(vertex_prog, fragment_prog);
	
	// PHONG SHADING
	fragment_prog = leer_codigo_de_fichero("./shaders/phong.fs");
	prog[3] = Compile_Link_Shaders(vertex_prog, fragment_prog);

	// BLINN-PHONG SHADING
	// vertex shader is the same than phong
	fragment_prog = leer_codigo_de_fichero("./shaders/blinn-phong.fs");
	prog[4] = Compile_Link_Shaders(vertex_prog, fragment_prog);

	fragment_prog = leer_codigo_de_fichero("./shaders/gooch.fs");
