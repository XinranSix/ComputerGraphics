...
static void glfw_error_callback(int error, const char* desc) { fputs(desc, stderr); }
...
GLWrapper::GLWrapper(bool fullScreen) { this->fullScreen = fullScreen; }
...
int GLWrapper::getWidth() { return width; }

int GLWrapper::getHeight() { return height; }

GLuint GLWrapper::getProgramId() { return shader.ID; }
...
void GLWrapper::init_shaders(rt_defines& defines) {
    const std::string vertexShaderSrc = readStringFromFile("./assets/shaders/quad.vert");
    std::string fragmentShaderSrc = readStringFromFile("./assets/shaders/rt.frag");
