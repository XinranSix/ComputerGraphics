...
enum SMAA_PRESET { LOW, MEDIUM, HIGH, ULTRA };

class SMAA_Builder {
public:
    SMAA_Builder(int width, int height, SMAA_PRESET preset) {
        smaa_shader_body = readStringFromFile("./assets/shaders/SMAA.h");
        ...
        snprintf(buff, sizeof(buff),
                 "\n#define SMAA_RT_METRICS float4(1.0 / %d.0, 1.0 / %d.0, "
                 "%d.0, %d.0)\n#define %s",
                 width, height, width, height, preset_str.c_str());
        header = glsl_header + std::string(buff);
    }

    void init_edge_shader(Shader& shader) const { shader.initFromSrc(create_edge_vs(), create_edge_ps()); }

    void init_blend_shader(Shader& shader) const { shader.initFromSrc(create_blend_vs(), create_blend_ps()); }
    ...
    static GLuint load_search_texture() {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, SEARCHTEX_WIDTH, SEARCHTEX_HEIGHT, 0, GL_RED, GL_UNSIGNED_BYTE,
                     searchTexBytes);
        glBindTexture(GL_TEXTURE_2D, 0);
...
private:
    ...
    std::string create_edge_vs() const { return header + header_vs + smaa_shader_body + edge_vs; }

    std::string create_edge_ps() const { return header + header_ps + smaa_shader_body + edge_ps; }

    std::string create_blend_vs() const { return header + header_vs + smaa_shader_body + blend_vs; }

    std::string create_blend_ps() const { return header + header_ps + smaa_shader_body + blend_ps; }

    std::string create_neighborhood_vs() const { return header + header_vs + smaa_shader_body + neighborhood_vs; }

    std::string create_neighborhood_ps() const { return header + header_ps + smaa_shader_body + neighborhood_ps; }
